...
if (!nv_function_exists('nv_menu_sharcode')) {
    /**
     * nv_menu_sharcode_getdata()
     *
     * @param array $list
     * @param int   $parentid
     ...
     * @return array
     */
    function nv_menu_sharcode_getdata($list, $parentid, $block_config)
    {
        global $site_mods;
    ...
    /**
     * nv_menu_sharcode_getsub()
     *
     * @param array  $smenus
     * @param string $block_theme
     * @return string
     */
    function nv_menu_sharcode_getsub($smenus, $block_theme)
    {
        $xtpl = new XTemplate('global.menusharecode.tpl', NV_ROOTDIR . '/themes/' . $block_theme . '/modules/menu');
        ...
        foreach ($smenus as $smenu) {
            ...
            if (!empty($smenu['sub'])) {
                $submenu = nv_menu_sharcode_getsub($smenu['sub'], $block_theme);
     ...
     * @return string
     */
    function nv_menu_sharcode($block_config)
    {
        ...
        $menulist = nv_menu_sharcode_getdata($list, 0, $block_config);
        ...
        if (!empty($menulist)) {
            foreach ($menulist as $menu) {
                ...
                if (!empty($menu['sub'])) {
                    $submenu = nv_menu_sharcode_getsub($menu['sub'], $block_theme);
                    $xtpl->assign('SUB', $submenu);
...
if (defined('NV_SYSTEM')) {
    $content = nv_menu_sharcode($block_config);
}
