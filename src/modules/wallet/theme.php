...
function nv_theme_wallet_recharge($row_payment, $post, $array_money_unit)
{
    ...
    if ($row_payment['payment'] == 'ATM' or $row_payment['payment'] == 'VietQR') {
        ...
        if ($is_vietqr) {
            $xtpl->assign('SCAN_MESSAGE', $row_payment['payment'] == 'ATM' ? $nv_Lang->getModule('atm_vietqr_scan') : $nv_Lang->getModule('vietqr_scan'));

            // Xử lý câu thông báo số tiền hợp lệ
            $array_amount = $module_config[$module_name]['minimum_amount'][$post['money_unit']];
            $array_amount = array_filter(explode(',', $array_amount));
            if (empty($array_amount)) {
                $xtpl->assign('MONEY_AMOUNT_RULE', $nv_Lang->getModule('atm_money_amount_true1'));
            } else {
                $xtpl->assign('MONEY_AMOUNT_RULE', sprintf($nv_Lang->getModule('atm_money_amount_true2'), get_display_money($array_amount[0])));
            }
