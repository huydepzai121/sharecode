<div class="card mb-4">
    <div class="card-body">
        <div class="row g-1">
            <div class="col-auto">
                <a class="btn btn-{(!$COMPLETE and !$INCOMPLETE) ? 'primary' : 'secondary'}" href="{$smarty.const.NV_BASE_ADMINURL}index.php?{$smarty.const.NV_LANG_VARIABLE}={$smarty.const.NV_LANG_DATA}&{$smarty.const.NV_NAME_VARIABLE}={$MODULE_NAME}&{$smarty.const.NV_OP_VARIABLE}={$OP}">Tất cả từ khóa</a>
            </div>
            <div class="col-auto">
                <a class="btn btn-{$COMPLETE ? 'primary' : 'secondary'}" href="{$smarty.const.NV_BASE_ADMINURL}index.php?{$smarty.const.NV_LANG_VARIABLE}={$smarty.const.NV_LANG_DATA}&{$smarty.const.NV_NAME_VARIABLE}={$MODULE_NAME}&{$smarty.const.NV_OP_VARIABLE}={$OP}&complete=1">Đã có mô tả</a>
            </div>
            <div class="col-auto">
                <a class="btn btn-{$INCOMPLETE ? 'primary' : 'secondary'}" href="{$smarty.const.NV_BASE_ADMINURL}index.php?{$smarty.const.NV_LANG_VARIABLE}={$smarty.const.NV_LANG_DATA}&{$smarty.const.NV_NAME_VARIABLE}={$MODULE_NAME}&{$smarty.const.NV_OP_VARIABLE}={$OP}&incomplete=1">Chưa có mô tả</a>
            </div>
            <div class="col-auto">
                <a class="btn btn-secondary" href="#" data-toggle="add_tags" data-fc="addTag" data-mtitle="Thêm từ khóa" data-tid="0">Thêm từ khóa</a>
            </div>
            <div class="col-auto">
                <a class="btn btn-secondary" href="#" data-bs-toggle="modal" data-bs-target="#mdTagMulti">Thêm nhiều từ khóa</a>
            </div>
        </div>
    </div>
</div>
            ...
            <div class="d-flex align-items-end flex-wrap justify-content-between g-3">
                <div>
                    <div class="input-group">
                        <input type="text" class="form-control" name="q" value="{$Q}" maxlength="64" placeholder="Tìm kiếm từ khóa" aria-label="Tìm kiếm từ khóa">
                        <button type="submit" class="btn btn-primary text-nowrap"><i class="fa-solid fa-magnifying-glass"></i> Tìm kiếm</button>
                    </div>
                </div>
                <div>
                    {if $COMPLETE}Đã có mô tả{elseif $INCOMPLETE}Chưa có mô tả{else}Tất cả từ khóa{/if}: <strong class="text-danger">{$NUM_ITEMS}</strong>
                </div>
            </div>
        </form>
    </div>
        ...
        <div class="table-responsive-lg table-card" id="list-tags-items">
            <table class="table table-striped align-middle table-sticky mb-0">
                <thead class="text-muted">
                    <tr>
                        <th class="text-nowrap" style="width: 1%;">
                            <input type="checkbox" data-toggle="checkAll" data-type="tag" class="form-check-input m-0 align-middle" aria-label="Chọn tất cả">
                        </th>
                ...
                <tbody>
                    ...
                    <tr>
                        <td>
                            <input type="checkbox" data-toggle="checkSingle" data-type="tag" value="{$row.id}" class="form-check-input m-0 align-middle" aria-label="Chọn">
                        </td>
                        <td>
                        ...
                        <td class="text-center">
                            ...
                            <i class="fa-solid fa-triangle-exclamation text-danger" data-bs-toggle="tooltip" data-bs-title="Chưa có mô tả"></i>
                            {else}
                        ...
                        <td>
                            <div class="row g-1 flex-nowrap">
                                <div class="col-auto">
                                    <button type="button" class="btn btn-secondary btn-sm"{if empty($row.num_sources)} disabled{else} data-toggle="link_tags" data-tid="{$row.id}"{/if}><i class="fa-solid fa-tags" data-icon="fa-tags"></i> Liên kết: <strong>{$row.num_sources}</strong></button>
                                </div>
                                <div class="col-auto">
                                    <button type="button" class="btn btn-secondary btn-sm" data-toggle="add_tags" data-fc="editTag" data-mtitle="Sửa từ khóa" data-tid="{$row.id}"><i class="fa-solid fa-pen" data-icon="fa-pen"></i> Sửa</button>
                                </div>
                                <div class="col-auto">
                                    <button type="button" class="btn btn-danger btn-sm" data-toggle="nv_del_tag" data-tid="{$row.id}"><i class="fa-solid fa-trash" data-icon="fa-trash"></i> Xóa</button>
                                </div>
                            </div>
                        </td>
                    </tr>
        ...
        <div class="d-flex flex-wrap justify-content-between align-items-center">
            <div class="d-flex flex-wrap flex-sm-nowrap align-items-center">
                ...
                <div class="input-group me-1 my-1">
                    <button type="button" class="btn btn-danger" data-toggle="nv_del_check_tags"><i class="fa-solid fa-trash" data-icon="fa-trash"></i> Xóa</button>
                </div>
            </div>
            ...
            <div class="modal-body">
                <form action="{$smarty.const.NV_BASE_ADMINURL}index.php?{$smarty.const.NV_LANG_VARIABLE}={$smarty.const.NV_LANG_DATA}&{$smarty.const.NV_NAME_VARIABLE}={$MODULE_NAME}&{$smarty.const.NV_OP_VARIABLE}={$OP}" method="post" class="ajax-submit">
                    ...
                    <div class="text-center">
                        <button class="btn btn-primary" type="submit"><i class="fa-solid fa-floppy-disk" data-icon="fa-floppy-disk"></i> Lưu</button>
                    </div>
                </form>
            </div>
        </div>
                ...
                <form action="{$smarty.const.NV_BASE_ADMINURL}index.php?{$smarty.const.NV_LANG_VARIABLE}={$smarty.const.NV_LANG_DATA}&{$smarty.const.NV_NAME_VARIABLE}={$MODULE_NAME}&{$smarty.const.NV_OP_VARIABLE}={$OP}" method="post" class="ajax-submit">
                    ...
                    <div class="row mb-3">
                        ...
                        <div class="col-12 col-sm-8">
                            <input type="text" class="form-control" id="element_stag_name" name="name" value="" maxlength="100">
                    ...
                    <div class="row mb-3">
                        ...
                        <div class="col-12 col-sm-8">
                            <input type="text" class="form-control" id="element_stag_alias" name="alias" value="" maxlength="100">
                            <div class="form-text">Để trống sẽ tự động tạo từ tên tag</div>
                        </div>
                    </div>
                    <div class="row mb-3">
                        <label for="element_stag_description" class="col-12 col-sm-3 col-form-label text-sm-end">Mô tả</label>
                        <div class="col-12 col-sm-8">
                            <textarea class="form-control" id="element_stag_description" name="description" rows="5"></textarea>
                        </div>
                    </div>
                    <div class="row mb-3">
                        <label for="element_stag_weight" class="col-12 col-sm-3 col-form-label text-sm-end">Thứ tự</label>
                        <div class="col-12 col-sm-8">
                            <input type="number" class="form-control" id="element_stag_weight" name="weight" value="0" min="0">
                        </div>
                    </div>
                    <div class="row">
            ...
            <div class="modal-footer justify-content-start">
                <div class="tag-tools">
                    <div class="d-flex align-items-center">
                        <div class="me-2">
                            <input type="checkbox" data-toggle="checkAll" data-type="link" class="form-check-input m-0 d-block" aria-label="Chọn tất cả">
                        </div>
                        <div>
                            <button type="button" class="btn btn-danger" data-toggle="tags_id_check_del" data-tid="0"><i class="fa-solid fa-trash" data-icon="fa-trash"></i> Xóa</button>
                        </div>
                    </div>
                </div>
            </div>
