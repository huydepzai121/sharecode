/**
 * NukeViet Content Management System - Users Module JavaScript
 * Enhanced for Bootstrap 5 with modern ES6+ features
 * @version 5.x
...
// Modern Users Module JavaScript with Bootstrap 5 Integration
class UsersModule {
    constructor() {
        this.init();
    }
    ...
    initAvatarUpload() {
        const avatarInputs = document.querySelectorAll('input[type="file"][data-avatar]');
        avatarInputs.forEach(input => {
            input.addEventListener('change', (e) => {
                this.previewAvatar(e.target);
            });
        });
    }
    ...
    bindEvents() {
        ...
        document.addEventListener('submit', (e) => {
            if (e.target.matches('[data-toggle="userLogin"]')) {
        ...
        document.addEventListener('submit', (e) => {
            if (e.target.matches('[data-toggle="reg_validForm"]')) {
                this.handleRegisterSubmit(e);
            }
        });

        // Modern AJAX handlers
        document.addEventListener('click', (e) => {
            if (e.target.matches('[data-toggle="admindeluser"]')) {
                this.handleDeleteUser(e);
            }
        });
    }

    // Enhanced login submission with loading states
    handleLoginSubmit(e) {
        e.preventDefault();
        const form = e.target;
    ...
    // Enhanced registration with progress indication
    handleRegisterSubmit(e) {
        e.preventDefault();
        const form = e.target;
        this.showRegistrationProgress(form);
    }

    showRegistrationProgress(form) {
    ...
    // Modern delete user confirmation
    handleDeleteUser(e) {
        e.preventDefault();
        const button = e.target.closest('[data-toggle="admindeluser"]');
    ...
    deleteUser(data) {
        ...
        .then(result => {
            if (result === 'OK') {
                ...
                const errorParts = result.split("_");
                if (errorParts[0] === 'ERROR') {
                    alert(errorParts[1]);
                } else {
...
function validErrorShow(a) {
    ...
    // Bootstrap 5 compatibility: use dispose instead of destroy
    if (typeof bootstrap !== 'undefined') {
        $("[data-mess]", $(a).parent().parent().parent()).not(".tooltip-current").each(function() {
...
function validErrorHidden(a, b) {
    ...
    // Bootstrap 5 compatibility: use dispose instead of destroy
    if (typeof bootstrap !== 'undefined') {
        var tooltip = bootstrap.Tooltip.getInstance(d[0]);
        if (tooltip) {
            tooltip.dispose();
        }
    } else {
