...
function nv_site_theme($contents, $full = true)
{
    ...
    if ($module_name != 'sharecode' and $module_name != 'home' and $module_name != 'users') {
    ...
    $html_links[] = [
        'rel' => 'stylesheet',
        'href' => NV_STATIC_URL . 'themes/' . $global_config['module_theme'] . '/bs5/css/bootstrap.min.css'
    ];

    // FontAwesome của hệ thống NukeViet
    $html_links[] = [
        'rel' => 'stylesheet',
    ...
    $html_links[] = [
        'rel' => 'stylesheet',
        'href' => NV_STATIC_URL . 'themes/' . $global_config['module_theme'] . '/css/aos.css'
    ];

    // Swiper.js Local CSS for modern sliders
    $html_links[] = [
        'rel' => 'stylesheet',
        'href' => NV_STATIC_URL . 'themes/' . $global_config['module_theme'] . '/swiper/swiper-bundle.min.css'
    ];
    ...
    // Custom.css chứa hero banner CSS và breadcrumb styling
    $html_links[] = [
        'rel' => 'stylesheet',
    ...
    // Bootstrap 5 Local JS Bundle (includes Popper.js)
    $html_js[] = [
        'ext' => 1,
        'content' => NV_STATIC_URL . 'themes/' . $global_config['module_theme'] . '/bs5/js/bootstrap.bundle.min.js'
    ];

    // AOS.js Local for animations
    $html_js[] = [
        'ext' => 1,
        'content' => NV_STATIC_URL . 'themes/' . $global_config['module_theme'] . '/js/aos.js'
    ];

    // Swiper.js Local for modern sliders
    $html_js[] = [
        'ext' => 1,
        'content' => NV_STATIC_URL . 'themes/' . $global_config['module_theme'] . '/swiper/swiper-bundle.min.js'
    ];

    // SweetAlert2 for modern alerts
    $html_js[] = [
        'ext' => 1,
        'content' => 'https://cdn.jsdelivr.net/npm/sweetalert2@11'
    ];
    ...
    foreach ($html_js as $js) {
        ...
        if ($js['ext']) {
            ...
            // Add integrity and crossorigin attributes for CDN scripts
            $xtpl->assign('JS_INTEGRITY', !empty($js['integrity']) ? (' integrity="' . $js['integrity'] . '"') : '');
            $xtpl->assign('JS_CROSSORIGIN', !empty($js['crossorigin']) ? (' crossorigin="' . $js['crossorigin'] . '"') : '');
    ...
    if ($full) {
        ...
        // Hero Section và Breadcrumbs
        if ($home) {
            $xtpl->parse('main.hero_section');
        } else {
            // Hiển thị breadcrumbs khi không ở trang home
        ...
        // Current time cho trang home khi không có mod title
